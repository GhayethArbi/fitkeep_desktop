...
public class selectActivitesFXML implements Initializable {
    ...
    public void displayActivites() {
        try {
            ...
            Mygrid.setHgap(20); // Adjust the value as needed
            Mygrid.setVgap(20); // Adjust the value as needed
    ...
    // Calculate the number of columns based on the size of selectedActivities
    private int calculateNumCols(int numItems) {
        // You can adjust the logic here based on your layout requirements
        // For example, you can set a maximum number of columns or calculate based on grid width
        return numItems; // Set the number of columns equal to the number of items
    }
    ...
    public void addActivity(MouseEvent mouseEvent) throws IOException {
        ...
        try {
            //System.out.println("----------");
            sapActivite.insertActivityWithidObjectif(activitePhysique,IdObj);
            Objectif objectif =sapActivite.fetchObjectifById(IdObj) ;
            objectif.setTotalCalories(objectif.getTotalCalories()+activitePhysique.getCaloriesBrules());
            objectif.setTotalDuree(objectif.getTotalDuree()+activitePhysique.getDureeActivite());
